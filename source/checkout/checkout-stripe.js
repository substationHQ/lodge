(function() {
                ...
                vv.loadScript('https://js.stripe.com/v2/', function() {
                    ...
                    if (!vv.stripe.eventAttached) {
                        vv.events.add(vv,'userinput', function(e) {
                            if (e.detail['vv__userinput-type'] == 'getstripetoken') {
                                ...
                                Stripe.card.createToken({
                                    ...
                                    if (response.error) {
                                        ...
                                        document.getElementById('vv__userinput-message').innerHTML = response.error.message;
                                        vv.styles.addClass(document.getElementsByClassName('vv__userinput-container')[0],'nope');
                                        setTimeout(function(){
                                            vv.styles.removeClass(document.getElementsByClassName('vv__userinput-container')[0],'nope');
                                        }, 800);
                                    } else {
                                        ...
                                        vv.overlay.reveal('<div class="vv__loading"></div>');
                                    }
                                });
    ...
    vv.userinput = {
        getInput: function (elements,type,style,msg) {
            ...
            container.className = 'vv__userinput-container';
            var message = document.createElement('div');
            message.id = 'vv__userinput-message';
            message.innerHTML = '&nbsp;';

            if (msg) {
                message.innerHTML = msg;
            }
            form.className = 'vv__userinput ' + type + ' ' + style;

            elements.push({id:'vv__userinput-type', type:'hidden', value:type});
            ...
            for (var i = 0; i < elements.length; i++) {
                ...
                input.id = "vv__userinput-" + type + "-" + element.id;
                if (element.required) {
                ...
                if (element.id == "stripe-submit" && vv.storage.checkoutdata.total) {
                    ...
                    total.innerHTML = '<h2 class="vv__pricing">Transaction amount: <span>'+vv.storage.checkoutdata.total+vv.storage.checkoutdata.transaction_message+"</span></h2><!--vv__pricing-->";
            ...
            vv.events.add(form,'submit', function(e) {
                ...
                if (incomplete) {
                    // Show the errors on the form
                    document.getElementById('vv__userinput-message').innerHTML = 'Please complete all required fields.';
                    vv.styles.addClass(document.getElementsByClassName('vv__userinput-container')[0],'nope');
                    setTimeout(function(){
                        vv.styles.removeClass(document.getElementsByClassName('vv__userinput-container')[0],'nope');
                    }, 800);
    ...
    vv.checkout = {
            ...
            if (vv.embedded) {
                ...
                if (options.shipping) {
                    ...
                    if (options.stripe || options.paypal) {
                        ...
                        // wait for them
                        vv.events.add(vv,'userinput', function(e) {
                            if (e.detail['vv__userinput-type'] == 'getshippingaddress') {
        ...
        showerror: function (type) {
            vv.overlay.reveal('<div class="vv__checkout-error">There are no valid payment types. Please add a payment connection. Check to make sure your site supports SSL (https) if you are using Stripe.</div>');
        }
    }
